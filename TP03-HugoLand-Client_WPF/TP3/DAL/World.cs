...
namespace HugoWorld
{
    ...
    public class World : GameObject
    {
        ...
        public World(GameState gameState, Dictionary<string, Tile> tiles, Monde monde)
        {
            _gameState = gameState;
            _tiles = tiles;
            sizeX = monde.LimiteX;
            sizeY = monde.LimiteY;
            hero = service.RetournerHero(Constantes.ID_hero);
            _heroPosition.X = hero.X;
            _heroPosition.Y = hero.Y;
            ...
            _heroPosition = new Point(_heroPosition.X < 0 ? 0 : _heroPosition.X, _heroPosition.Y < 0 ? 0 : _heroPosition.Y);
        ...
        //BD à la place d'un fichier
        private void readMapObject(Monde monde)
        {
            List<ObjetMonde> listeObj = service.ListerObjetPourMonde(monde.Id);
            List<Item> listeItems = service.ListerItemsPourMonde(monde.Id);
            ...
            for (int i = 0; i < sizeX / 8; i++)
            {
                ...
                {
                    Area area = new Area(listeObj, listeItems, _tiles, i, j);
                    if (i == 0 && j == 0)
                    {
                        area.Name = "start";
                    }
        ...
        {
            List<Hero> liste = service.ListerHeroConnecter(Constantes.ID_monde);
        ...
        private void checkObjects()
        {
            ...
            switch (objectTile.Category)
            {
                ...
                case "armour":
                    _gameState.Armour++;
                    service.ModifierInventaireHero(Constantes.ID_monde, int.Parse(objectTile.TypeID), Constantes.ID_hero, _heroPosition.X, _heroPosition.Y);
                    Sounds.Pickup();
                    break;
            ...
            if (objectTile.Category != "fire" && objectTile.Category != "bones" && objectTile.Category != "character" && objectTile.Category != "normal" && objectTile.AttributeType == "item")
            {
            ...
            if (!_heroSpriteAnimating && !_heroSpriteFighting)
            {
                switch (key)
                {
                    case Key.Right:
                        ...
                        else if (_currentArea.EastArea != null)
                        {
                    ...
                    case Key.Left:
                        ...
                        else if (_currentArea.WestArea != null)
                        {
                    ...
                    case Key.Up:
                        ...
                        else if (_currentArea.NorthArea != null)
                        {
                            //Edge of map - move to next area
                    ...
                    case Key.Down:
                        ...
                        else if (_currentArea.SouthArea != null)
                        {
                            //Edge of map - move to next area
                    ...
                    case Key.P:
                        ...
                        if (_gameState.Potions > 0)
                        {
                            ...
                            _gameState.Potions--;
                            service.ModifierInventaireHero(Constantes.ID_monde, 48, Constantes.ID_hero, _heroPosition.X, _heroPosition.Y);
        ...
        private bool checkNextTile(MapTile mapTile, int x, int y)
        {
            ...
            if (mapTile.ObjectTile != null && mapTile.ObjectTile.Category == "character")
            {
                ...
                if (_random.Next(0, _gameState.Attack + 1) * (Constantes.Hero.StatDex / 4) * (Constantes.Hero.StatStr / 4) >= (mapTile.ObjectTile.Health / 5))
                {
                    //Hero damage is up to twice the attack rating
                    if (damageMonster(_random.Next(0, _gameState.Attack + 1) * (Constantes.Hero.StatDex/4) * (Constantes.Hero.StatStr/4), mapTile, x, y))
                    {
                        //Monster is dead now
                        return true;
                    }
            ...
            if (mapTile.Tile is null) return false;
